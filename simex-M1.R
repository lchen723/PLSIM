...
method <- "simex"  # c("naive", "simexboost", "simex")
mmm <- "M1"  # c("M1", "M2")
...
for(i in 1:100){
  ...
  #total_vars = NULL
  #for(k in 1:tot_vars_p) {
  #total_vars = rbind(total_vars, runif(n,-1,1))
  # }
  ...
  beta <- c(rep(0.6,2),rep(0,X_vars_px-2))
  ...
  #### (2-2-3) q_star (all k, zi 3i??FFG-n-poai??FFDX!LO) ####
  ...
  for(zi_m in 0:(M-1)) {
    ...
    for(k in 1:K) {
      ...
      for(t in 1:70) {
        ...
        #      xi_gamma <- 0.0001    ### !!OUi??FFDy?A3]cw
        xi_gamma <- 0.0004    ### !!OUi??FFDy?A3]cw
        if(length(which((gamma_candidate<0))) == 6){
          id_gamma = which((gamma_candidate) >= 0.0*max((gamma_candidate)))}      
        if(length(which((gamma_candidate<0))) != 6){
          id_gamma = which((gamma_candidate) >= 0.0*max((gamma_candidate))) }
    ...
    #gamma_zeta[which(abs(gamma_zeta)<0.6)] = 0
  }
...
spe_beta <- sum(which(ifelse(estB < 0.1, 0, estB)==0)>2)/(X_vars_px-2) # specificity of beta M1
sen_beta <- sum(which(ifelse(estB < 0.1, 0, estB)!=0)<=2)/2 # sensitivity of beta M1
